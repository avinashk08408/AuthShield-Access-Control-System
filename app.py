...
<nav class="mobile-drawer" id="mobileDrawer">
  ...
  <div class="drawer-footer">
    <a href="/logout" style="color:var(--danger);"><span class="nav-icon">🚪</span>Sign Out</a>
  </div>
</nav>
<header class="topbar">
  <div style="display:flex;align-items:center;gap:12px;">
    ...
    <div class="topbar-brand">
      ...
      <div>AuthShield<span class="brand-sub">Access Control System</span></div>
    </div>
  </div>
...
<div class="layout">
  <nav class="sidebar">
    ...
    {admin_sidebar}
    <hr class="sidebar-divider">
    <a href="/logout" style="color:var(--danger);margin-top:4px;"><span class="nav-icon">🚪</span>Sign Out</a>
  </nav>
...
def edit_user(target):
    ...
    if request.method == "POST":
        ...
        if new_role not in ("Admin", "User"):
            ...
            conn = get_db()
            cur  = conn.cursor()
            cur.execute("UPDATE users SET role=? WHERE username=?", (new_role, target))
            conn.commit()
            cur.close()
            conn.close()
...
def unlock_user(target):
    conn = get_db()
    cur  = conn.cursor()
    cur.execute("UPDATE users SET locked=0, failed=0 WHERE username=?", (target,))
    conn.commit()
    cur.close()
    conn.close()
    ...
    if request.method == "POST":
        ...
        if not u:
            ...
            if not valid:
                ...
                conn = get_db()
                cur  = conn.cursor()
                cur.execute("UPDATE users SET password=?, failed=0, locked=0 WHERE username=?",
